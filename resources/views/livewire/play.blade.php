...
<div>
    ...
    <div class="flex gap-x-2 justify-between items-stretch justify-stretch relative mt-1 gap-x-5 mb-3">
        <div class="w-2/3 bg-gray-950 border-l-2 border-b-2 border-gray-800 flex items-center rounded-lg">
            <div class="mx-2 text-center cursor-pointer uppercase text-gray-200 bg-gray-900 text-[.6rem] rounded-lg bottom-2 right-1 py-1 px-2">NAME:</div>
            <input wire:model="name" type="text" class="font-black py-2 tracking-wide px-1 focus:ring-0 border-0 bg-transparent text-md w-full text-gray-400" />
        </div>
        <div class="flex items-center w-1/3 bg-gray-950 border-l-2 border-b-2 border-gray-800 flex justify-between items-center rounded-lg">
            <div class="mx-2 text-center cursor-pointer uppercase text-gray-200 bg-gray-900 text-[.6rem] rounded-lg bottom-2 right-1 py-1 px-2">AVATAR:</div>
            <img wire:click="changeAvatar" title="click to change" class="cursor-pointer w-8 h-8 mx-2" src="{{ asset('images/player/' . $avatarList[$avatarIndex]) }}" />
        </div>        
    </div>
    ...
    @elseif($money > 5000)
        You're on fire! Try your luck with <span class="text-green-600"><a target="_blank" href="https://www.pesowin.xyz/c-ViXcYCRz?lang=en" class="underline">real mines</span></a>.
    @else
